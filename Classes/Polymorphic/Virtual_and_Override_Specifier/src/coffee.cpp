...
public:
    ...
    DEMO_MEMBER_ANCHOR(polymorphic_virtual_content)
    virtual std::string content() const {
        return brand + " in " + cup_to_string(cup);
    }

    DEMO_MEMBER_ANCHOR(polymorphic_virtual_change_cup)
    virtual bool change_cup(cup_e const to_cup) {
        cup = to_cup;
        return true;
    }
...
public:
    ...
    DEMO_MEMBER_ANCHOR(override_milk_content)
    std::string content() const override {
        return coffee_t::content() + " with milk " + std::to_string(milk);
    }

    DEMO_MEMBER_ANCHOR(override_milk_change_cup)
    bool change_cup(cup_e const to_cup) override {
...
public:
    ...
    DEMO_MEMBER_ANCHOR(override_sugar_content)
    std::string content() const override {
...
DEMO(VirtualPureSpecifier, Correct) { // NOLINT(cert-err58-cpp)
    ...
    std::cout << "output: " << coffee_1->content() << std::endl; // correct

    std::cout << std::endl;

    std::cout << "expect: false" << std::endl;
    std::cout << "output: "
              << coffee_1->change_cup(coffee_t::MIDDLE_CUP) // correct
              << std::endl;
