 ...
 * Base class
 */
DEMO_ANCHOR(coffee_multiple_inheritance)
class coffee_t {
public:
    enum cup_e {
        SUPER_CUP,
...
DEMO(MultipleInheritance, SomethingWrong) { // NOLINT(cert-err58-cpp)

    class try_with_milk_and_sugar_t : public with_milk_t, public with_sugar_t {
    public:
        try_with_milk_and_sugar_t(std::string const& brand, cup_e cup, double milk, double sugar)
                //: coffee_t(brand, cup)
    ...
    try_with_milk_and_sugar_t coffee("Nescafe", coffee_t::BIG_CUP, .1, .2);
